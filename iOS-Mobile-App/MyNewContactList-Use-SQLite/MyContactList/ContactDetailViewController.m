...
- (void) getPersonalInfos:(NSString *)person
{
    ...
    NSLog(@"ok1");
    NSArray* tempPerson = [NSArray arrayWithObject:person];
    NSLog(@"ok2: %@", tempPerson);
    //NSString* newPerson = [[NSString alloc]initWithFormat:@"%@ %@", [tempPerson objectAtIndex:1], [tempPerson objectAtIndex:0]];
    NSMutableArray* myPerson = [[StorageContact sharedInstance]getContact:person];//call method to get all personal infos
...
- (void) accessFileAlert
{
    //alert user open file has problem
    UIAlertController* alertScreen = [UIAlertController alertControllerWithTitle:@"ERROR" message:@"Problem access data!" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction* okButton = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction* action)
                               {
                                   [alertScreen dismissViewControllerAnimated:YES completion:nil];
                               }];
...
- (IBAction)saveEditButtonTapped:(id)sender
{
    ...
    if (self.myNameInView2 == nil)//if user tapped add sign
    {
        ...
        //////for test only
        //[[StorageContact sharedInstance]testAddContact];
        ///////
        ...
        if (addResult == true)//user enter fits all the requirements
        {
            NSLog(@"add ok");
            NSLog(@"2");
            checkIfFileOpenProblem = [[StorageContact sharedInstance]addContact:mycontact];// call addcontact method to add information to file
            if (checkIfFileOpenProblem == true)
            {
                NSLog(@"add to DB file ok in view 2");
            }
            else
            {
                NSLog(@"3");
                [self accessFileAlert];
            }
        }
        else
        {
